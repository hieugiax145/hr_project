...
import { Form, Input, Select, Button, Space, Typography, DatePicker, Upload, Radio, Table, Checkbox, message, Spin } from 'antd';
import { useNavigate } from 'react-router-dom';
import { ArrowLeftOutlined, PlusOutlined, DeleteOutlined, LoadingOutlined } from '@ant-design/icons';
...
const CreateNotification = () => {
  ...
  const onFinish = async (values) => {
    try {
      ...
      const personalPhotoSize = values.personalPhoto.fileList[0].originFileObj.size;
      const idCardPhotosSize = values.idCard.photos.fileList.reduce((total, file) => total + file.originFileObj.size, 0);
      
      if (personalPhotoSize > 5 * 1024 * 1024) { // 5MB
        message.error('Ảnh cá nhân không được vượt quá 5MB');
        return;
      }
      
      if (idCardPhotosSize > 10 * 1024 * 1024) { // 10MB
        message.error('Ảnh CMND/CCCD không được vượt quá 10MB');
        return;
      }
      ...
      try {
        ...
        if (error.message.includes('timeout')) {
          message.error('Yêu cầu đã hết thời gian chờ. Vui lòng kiểm tra kết nối mạng và thử lại.');
        } else if (error.message.includes('kết nối')) {
          message.error('Không thể kết nối đến máy chủ. Vui lòng kiểm tra kết nối mạng và thử lại.');
        } else {
          message.error(error.message || 'Lỗi khi tạo thông báo');
        }
      }
    } catch (error) {
  ...
  return (
    <div className="p-6 pt-[104px] pl-[298px]">
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <Spin spinning={isLoading || isSubmitting} tip={isLoading ? "Đang tải dữ liệu..." : "Đang tạo thông báo..."} indicator={<LoadingOutlined style={{ fontSize: 24 }} spin />}>
          <Form
            form={form}
            layout="vertical"
            onFinish={onFinish}
          ...
          >
            ...
            <div className="flex justify-end gap-4">
              <Button onClick={() => navigate('/notifications')} disabled={isSubmitting || isLoading}>
                Hủy
              </Button>
              <Button type="primary" htmlType="submit" loading={isSubmitting} disabled={isLoading}>
